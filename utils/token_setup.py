...
def generate_token():
    ...
    if token == '':
        ...
        with open(str(config.get('File Paths', 'token_file_path')), 'w') as token_file:
...
def check_for_token():
    ...
    if not os.path.exists(config.get('File Paths', 'token_file_path')):
        logger.warning('[Token Checker]: Keyfile not found!')
        generate_token()

    else:
        with open(config.get('File Paths', 'token_file_path'), 'r') as token_file:
